...
export interface DatePickerProps
    ...
    enableYear?: number // Prop to enable only a specific year
}
...
const DatePicker = (props: DatePickerProps) => {
    ...
    // Calculate min and max dates based on enableYear prop
    const [calculatedMinDate, calculatedMaxDate] = useMemo(() => {
        if (enableYear !== undefined) {
            // Set min date to January 1st of the enabled year
            const minDate = new Date(enableYear, 0, 1)
            // Set max date to December 31st of the enabled year
            const maxDate = new Date(enableYear, 11, 31)
            return [minDate, maxDate]
        }
        return [undefined, undefined]
    }, [enableYear])

    // Use prop minDate if provided, otherwise use calculated minDate
    const minDate = propMinDate || calculatedMinDate

    // Use prop maxDate if provided, otherwise use calculated maxDate
    const maxDate = propMaxDate || calculatedMaxDate
    ...
    // If enableYear is set and value is not in that year, adjust it
    useEffect(() => {
        if (enableYear !== undefined && _value instanceof Date) {
            const valueYear = _value.getFullYear()
            
            if (valueYear !== enableYear) {
                // Set to January 1st of the enabled year
                const newDate = new Date(enableYear, 0, 1)
                setValue(newDate)
            }
        }
    }, [enableYear, _value, setValue])

    const [calendarMonth, setCalendarMonth] = useState(
        _value || defaultMonth || new Date(),
    )

    // If enableYear is set, ensure calendarMonth is in the enabled year
    useEffect(() => {
        if (enableYear !== undefined) {
            const calendarYear = calendarMonth.getFullYear()
            if (calendarYear !== enableYear) {
                // Set to January of the enabled year
                setCalendarMonth(new Date(enableYear, 0, 1))
            }
        }
    }, [enableYear, calendarMonth])
    ...
    const handleValueChange = (date: Date | null) => {
        if (date instanceof Date && enableYear !== undefined) {
            const dateYear = date.getFullYear()
            
            // Check if date is not in the enabled year
            if (dateYear !== enableYear) {
                // Prevent selection of date not in the enabled year
                return
            }
        }
    ...
    const setDateFromInput = () => {
        ...
        // If enableYear is set, check if date is in the enabled year
        if (enableYear !== undefined && date instanceof Date) {
            const dateYear = date.getFullYear()
            
            if (dateYear !== enableYear) {
                // Set to January 1st of the enabled year
                date = new Date(enableYear, 0, 1)
            }
        }
    ...
    const handleChange = (event: ChangeEvent<HTMLInputElement>) => {
        ...
        if (dayjs(date).isValid()) {
            // Check if enableYear is set and date is not in that year
            if (enableYear !== undefined) {
                const dateYear = date.getFullYear()
                
                if (dateYear !== enableYear) {
                    // Don't update if date is not in the enabled year
                    return
                }
            }
    ...
    // Enhanced disableDate function to include the enableYear restriction
    const enhancedDisableDate = (date: Date) => {
        // First apply the custom disableDate function if provided
        if (disableDate && disableDate(date)) {
            return true
        }

        // Then apply the enableYear logic
        if (enableYear !== undefined) {
            const dateYear = date.getFullYear()
    ...
    return (
        ...
        >
            <Calendar
                ...
                minDate={minDate}
                maxDate={maxDate}
                disableDate={enhancedDisableDate}
